...
function ensure() {
  ...
  Database.use((db: DbClient) => {
    ...
    `)
    db.run(sql`CREATE INDEX IF NOT EXISTS agentboard_run_project_issue_idx ON agentboard_run(project_id, issue_id)`)
    db.run(sql`
    ...
    `)
    db.run(sql`CREATE INDEX IF NOT EXISTS agentboard_artifact_run_idx ON agentboard_artifact(run_id)`)
    db.run(sql`
    ...
    `)
    db.run(sql`CREATE INDEX IF NOT EXISTS agentboard_run_event_run_idx ON agentboard_run_event(run_id, time_created)`)
    db.run(sql`
  ...
  deleteArtifacts(runID: string, kinds?: AgentBoardArtifactKind[]) {
    ...
    Database.use((db: DbClient) => {
      ...
      for (const kind of kinds) {
        db.run(sql`DELETE FROM agentboard_artifact WHERE run_id = ${runID} AND kind = ${kind}`)
      }
    })
  },
  addArtifact(input: {
